...
void frame_new(void *data, struct wl_callback *cb, uint32_t a) {
  struct app_state *app = (app_state *)data;
...
int main() {
  ...
  xdg_surface_add_listener(xrfc, &xrfc_list, &app);
  surface_top_level = xdg_surface_get_toplevel(xrfc);
  xdg_toplevel_add_listener(surface_top_level, &toplevel_listener, &app);
  ...
  app.GLState.uniformOffset = glGetUniformLocation(prog, "offset");
  GLuint vbo;
  glGenBuffers(1, &vbo);
